...
| `s_visibility_proxy` | 弱能見度 proxy，越高代表退化越嚴重 | `[B, 1]` |
...
- `s_visibility_proxy`（V1 CSV 中的 legacy 名稱為 `s_haze`）：

  \[
  s_{visibility}=0.5s_{contrast}+0.3s_{low\_saturation}+0.2s_{flat\_brightness}
  \]
...
- `results/convnext_tiny_finetune/gradcam/392_s_haze.png`（V1 legacy filename）
